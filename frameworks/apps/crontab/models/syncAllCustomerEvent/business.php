...
class business
{
    ...
    public function __construct() 
    {
        ...
        // 实例化数据访问对象
		$this->_database_model = getInstance('model.syncAllCustomerEvent.database');
	 ...
	 * 
	 * 【说明】
	 * 执行命令：/usr/bin/php /usr/share/nginx/html/wechatApp/www/crontab/index.php syncAllCustomerEvent
	 * 执行时间：每分钟定时执行一次 * * * * *
	 * 
	 */
	...
	{
        // 获取需要执行的指定计划任务详情
		$crontabDetail = $this->_common_database_model->getCrontab('syncAllCustomers');
		...
		// 获取配置了客户联系功能的成员列表
		$followuserList = $this->_workWeixin->getFollowuserList();
		...
		if ($followuserList) {
			foreach ($followuserList as $key => $value) {
				...
				$followCustomerSingle = $this->_database_model->getFollowCustomerSingle($value);
