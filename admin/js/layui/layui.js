...
layui.define(["laytpl", "laypage", "form", "util"], function(e) {
    ...
    var m = layui.$
      ...
      , j = [, "{{# if(d.data.toolbar){ }}", '<div class="layui-table-tool">', '<div class="layui-table-tool-temp"></div>', '<div class="layui-table-tool-self"></div>', "</div>", "{{# } }}", '<div class="layui-table-box">', "{{# if(d.data.loading){ }}", '', '', "", "{{# } }}", "{{# var left, right; }}", '<div class="layui-table-header">', t(), "</div>", '<div class="layui-table-body layui-table-main">', a, "</div>", "{{# if(left){ }}", '<div class="layui-table-fixed layui-table-fixed-l">', '<div class="layui-table-header">', t({
        fixed: !0
    ...
    i.prototype.loading = function(e) {
        var t = this;
        t.config.loading && (e ? (t.layInit = m(['', '<script>layer.close(index);</script>', ""].join("")),
        t.layBox.append(t.layInit)) : (t.layInit = m(['', '<script>var index = layer.msg("加载中", {icon: 16,shade: 0.01});</script>', ""].join("")),
            ...
            case "LAYTABLE_COLS":
                l({
                    ...
                    s.eachCols(function(e, t) {
                        console.log(e,t)
                        // if (t.hide) {
                        //     t.field && "normal" == t.type && a.push('<li style="color:#c2c2c2!important"><input type="checkbox" name="' + t.field + '" data-key="' + t.key + '" data-parentkey="' + (t.parentKey || "") + '" lay-skin="primary" ' + (t.hide ? "" : "checked") + ' title="' + b.escape(t.title || t.field) + '" lay-filter="LAY_TABLE_TOOL_COLS" class="layui-checkbox-disabled layui-disabled" disabled></li>')
                        // }else{
